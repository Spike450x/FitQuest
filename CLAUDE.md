...
10. **Health-data integration** — _scaffold shipped (Garmin-direct/free, 2026-05-30; see above)._ Remaining: get Garmin Developer Program approval, set the Cloud Functions secrets (`GARMIN_CLIENT_ID`/`GARMIN_CLIENT_SECRET`/`GARMIN_WEBHOOK_TOKEN`) + `GARMIN_REDIRECT_URI`, register the OAuth redirect + Push callbacks, flip `NEXT_PUBLIC_HEALTH_SYNC_ENABLED=true`, and verify the post-approval constants in docs/HEALTH-INTEGRATION.md §8. **Apple Health** still needs a native iOS/Capacitor shell (HealthKit has no web API). A paid aggregator (Terra/Rook) remains a one-adapter upgrade on the provider-neutral core
