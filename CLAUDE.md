...
**Active focus:** Health-data integration scaffold landed (feature-flagged off). To enable, set the **Strava** secrets — the works-today free path, no approval needed — and flip `NEXT_PUBLIC_HEALTH_SYNC_ENABLED` (runbook in [docs/HEALTH-INTEGRATION.md](docs/HEALTH-INTEGRATION.md) §7). Garmin-direct (steps + sleep) waits on enterprise Developer Program approval; a paid aggregator like Terra/Rook remains a one-adapter upgrade on the provider-neutral core. Next on the roadmap: **Reputation / Wanted Board PR2** — the **Fight fork** (bounty-completion combat encounter for a bigger payout, wiring `claimBounty`'s deferred `path: 'fight'` seam to `useCombatEncounter`) + a hardening `claimBounty` Cloud Function + rank stat bonuses, building on the PR1 foundation now shipped. Spec: `docs/superpowers/specs/2026-05-17-future-features-roadmap-design.md`. The Achievements catalog view (locked/unlocked states, descriptions, gold reward display) already shipped as the default tab of `/collections`.
...
1. **Reputation / Wanted Board** — _PR1 foundation shipped (2026-05-31): dual-track Reputation currency, 5-tier rank, `/wanted` daily bounties, Loot claim path._ Remaining: **Fight fork** (combat-encounter payout via the `claimBounty` `path: 'fight'` seam), a hardening `claimBounty` CF, and rank stat bonuses. Spec: `2026-05-17-future-features-roadmap-design.md`
