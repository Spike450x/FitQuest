...
## Current Status (last updated 2026-05-29 — Content-scaling PR5a: Bestiary + Collection + quest expansion)
...
**Active focus:** PR5b — the **24 new achievements** (combat / activity / mastery / quest / collection) — is the remaining slice. It's server-authoritative (extends `claimCombatVictory` + `logActivity` CFs with transactional achievement merges + parity tests), so it was split out from the read-only PR5a surfaces to keep the risky Cloud-Function work isolated. After PR5b lands, run the dedicated `/balance-check` pass per the plan's guardrails.
