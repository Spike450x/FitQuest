...
**Health-data integration scaffold (2026-05-30)** — Foundation for auto-logging real activity from wearables via the **Terra** aggregator (Garmin / Fitbit / Oura / WHOOP / Strava / Google Fit / Polar / Samsung / Withings / Suunto). Feature-flagged off behind `NEXT_PUBLIC_HEALTH_SYNC_ENABLED` + three Cloud Functions secrets pending a Terra account; full design + runbook in [docs/HEALTH-INTEGRATION.md](docs/HEALTH-INTEGRATION.md). **Shared write core** — extracted `logActivityCore` (`functions/src/logActivityCore.ts`) from the `logActivity` callable so device-synced and manual logs run the _identical_ authoritative sequence (server-side daily cap → log write → mastery milestone → resource restore → achievements); the callable is now a thin auth+validation wrapper (no behaviour change, existing parity tests guard it). **New Cloud Functions** — `terraWebhook` (repo's **first `onRequest` HTTP function**: HMAC-SHA256 `terra-signature` verification over the raw body, payload→activity mapping, de-dupe, ingestion via the shared core) and `createTerraSession` (`onCall` widget-session generator bound to `reference_id = uid`). First use of Firebase Functions **secrets**. **Pure, tested modules** — `gameLogic/healthMapping.ts` (Terra `activity`/`daily`/`sleep` → run/workout/steps/sleep, clamped to `ACTIVITY_AMOUNT_MAX`), `gameLogic/healthDedupe.ts` (event-id idempotency + cumulative daily-step delta math via a `healthDailySnapshots` cursor), `terraSignature.ts`. **New collections** — `healthConnections` (owner-read, server-write-only) + `healthDailySnapshots` (server-only); rules deny all client writes since the admin-SDK webhook owns them. New optional `ActivityLog.source` field (e.g. `terra:GARMIN`) drives a "⌚ synced" badge in the dashboard feed. **Client surface** — `lib/health.ts`, `lib/healthData.ts`, `useHealthConnections`, `/profile/connections` page (feature-flagged), profile link. **CSP unchanged** (connect flow is a full-page nav to a Terra-hosted URL; webhook is inbound). **Apple Health intentionally out of scope** — HealthKit has no web API; needs a native iOS/Capacitor shell, which the Terra path is forward-compatible with. 29 new functions vitest specs (functions suite 16 → 45); 860 root tests still green; CI `build:ci` clean.
...
10. **Health-data integration** — _scaffold shipped (Terra aggregator, 2026-05-30; see above)._ Remaining: obtain a Terra account, set the three Cloud Functions secrets, register the webhook, flip `NEXT_PUBLIC_HEALTH_SYNC_ENABLED=true`. **Apple Health** still needs a native iOS/Capacitor shell (HealthKit has no web API) — separate, larger effort the Terra path is forward-compatible with
