...
## Current Status (last updated 2026-05-29 — Content-scaling PR4: Spells + bleed/burn DoT)
...
**Active focus:** Continuing the content-scaling drop. PR5 (achievements + bestiary surface + collection surface — both surfaces will consume the `monstersKilled` data PR2 is already writing and the inventory data PR3 is already growing) is the final slice, then a dedicated `/balance-check` pass per the plan's guardrails.
