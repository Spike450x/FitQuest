...
describe("resolveSpell", () => {
  ...
  it("deals damage after defense when requirement is met", () => {
    vi.spyOn(Math, "random")
      .mockReturnValue(0.5); // dice rolls + def checks all use 0.5
  ...
  it("scales damage with wisdom when damageScalesWithWisdom is true", () => {
    vi.spyOn(Math, "random").mockReturnValue(0.5);
    const effect: SpellEffect = { damage: 10, damageScalesWithWisdom: true, bypassMonsterDef: true };
    const char = makeChar({ stats: { strength: 10, stamina: 10, agility: 10, health: 10, wisdom: 15, defense: 5 } });
