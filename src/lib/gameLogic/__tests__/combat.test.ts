...
const BASE_STATS = {
  strength: 10, stamina: 10, agility: 10, health: 10, wisdom: 10, defense: 10,
};
...
describe("playerMaxHp", () => {
  it("matches BASE_HP + stamina*HP_PER_STAMINA + health*HP_PER_HEALTH", () => {
    ...
    const expected =
      COMBAT.BASE_HP +
      10 * COMBAT.HP_PER_STAMINA +
      10 * COMBAT.HP_PER_HEALTH;
...
describe("totalGearBonuses", () => {
  ...
  it("stacks bonuses from multiple slots", () => {
    ...
    const bonuses = totalGearBonuses({ weapon: "worn-sword", armor: "leather-vest", accessory: null });
...
describe("rollLoot", () => {
  ...
  it("applies streak multiplier to rare items but caps at 0.95", () => {
    ...
    const table = [{ itemId: "dragonbone-blade", chance: 0.10 }];
