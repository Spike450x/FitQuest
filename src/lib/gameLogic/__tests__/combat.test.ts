...
describe('combatXpDailyMultiplier', () => {
  it('returns 1.0× for the first 5 wins of the day', () => {
    ...
    expect(combatXpDailyMultiplier(3)).toBe(1.0);
    expect(combatXpDailyMultiplier(4)).toBe(1.0);
  });

  it('returns 0.5× for wins 6–15', () => {
    expect(combatXpDailyMultiplier(5)).toBe(0.5);
    expect(combatXpDailyMultiplier(10)).toBe(0.5);
    expect(combatXpDailyMultiplier(14)).toBe(0.5);
  });

  it('returns 0.25× for wins 16–25', () => {
    expect(combatXpDailyMultiplier(15)).toBe(0.25);
    expect(combatXpDailyMultiplier(20)).toBe(0.25);
    expect(combatXpDailyMultiplier(24)).toBe(0.25);
  });

  it('floors at 0.1× from win 26 onward', () => {
    expect(combatXpDailyMultiplier(25)).toBe(0.1);
...
describe('combatWinsUntilNextPenalty', () => {
  it('reports remaining wins to the 0.5× tier from a fresh day', () => {
    expect(combatWinsUntilNextPenalty(0)).toEqual({ remaining: 5, nextMultiplier: 0.5 });
    expect(combatWinsUntilNextPenalty(3)).toEqual({ remaining: 2, nextMultiplier: 0.5 });
  });

  it('reports remaining wins to the 0.25× tier inside the 0.5× band', () => {
    expect(combatWinsUntilNextPenalty(5)).toEqual({ remaining: 10, nextMultiplier: 0.25 });
    expect(combatWinsUntilNextPenalty(13)).toEqual({ remaining: 2, nextMultiplier: 0.25 });
  });

  it('reports remaining wins to the 0.1× floor inside the 0.25× band', () => {
    expect(combatWinsUntilNextPenalty(15)).toEqual({ remaining: 10, nextMultiplier: 0.1 });
    expect(combatWinsUntilNextPenalty(24)).toEqual({ remaining: 1, nextMultiplier: 0.1 });
  });

  it('returns null past the final tier', () => {
    expect(combatWinsUntilNextPenalty(25)).toBeNull();
