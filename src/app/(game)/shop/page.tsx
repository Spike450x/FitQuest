...
export default function ShopPage() {
  ...
  const filtered = DAILY_ITEMS.filter(
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Buy gear, consumables, and spells. Equip up to 5 spells before combat.</p>
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${RARITY_BADGE[item.rarity]}`}>
                      {item.rarity}
                    </span>
                  </div>
              ...
              {owned ? (
                ...
                >
                  {isBuying ? "Buying…" : !canAfford ? "Not enough gold" : `Buy for ${item.price} 💰`}
                </button>
              )}
            </div>
          );
