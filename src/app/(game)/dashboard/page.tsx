...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {(character.pendingStatPoints ?? 0) > 0 && (
    ...
    <li className="space-y-1">
      <div className="flex items-center justify-between gap-2">
        ...
        {isClaimed ? (
          ...
          <Link href="/quests" className="text-xs text-amber-600 font-semibold shrink-0 hover:underline">
            Claim! →
          </Link>
        ) : (
      ...
      <span className="text-xs text-gray-400">
        {quest.progress.toLocaleString()} / {def.requirement.target.toLocaleString()} {def.requirement.unit}
      </span>
    </li>
  );
}
