    ...
    try {
      ...
      if (typeof runId !== 'string' || runId.length === 0 || runId.length > 128) {
        throw new HttpsError(
          'invalid-argument',
          'runId must be a non-empty string (max 128 chars).',
        );
      }
      ...
      if (outcomeStatus !== 'completed' && outcomeStatus !== 'abandoned') {
        throw new HttpsError(
          'invalid-argument',
          'outcomeStatus must be "completed" or "abandoned".',
        );
      }
      ...
      return {
        xp: awardedXp,
        gold: awardedGold,
        achievementGold: earnedAchievements.reduce(
          (sum, id) => sum + (ACHIEVEMENT_GOLD[id] ?? 0),
          0,
        ),
        items: droppedItems,
      ...
      console.error('[claimDungeonRun] unhandled error:', err);
