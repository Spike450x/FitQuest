...
| `incomingMonsterDamage(character, monster, rawAttack, effectiveDef, special?)` | Final monster→player hit (pre-passives/dodge). **Physical** subtracts `effectiveDef`; **magic** ignores armor — then scaled by `CLASS_DAMAGE_TAKEN[class][school]` and floored at `MIN_DAMAGE`. The magic axis of the class triangle (Warrior ×1.3 magic, Wizard ×0.75 magic). An optional rolled `MonsterSpecialMove` reshapes the hit: `pierce` zeroes armor, `burst` makes it magic, `heavy` multiplies. Used by `calculateRound` / `rollMonsterAttack` / spell counter / recovery + failed-escape hits. |
| `rollMonsterSpecial(monster, rng?)`                                            | Rolls a monster's `specialMoves` on its counter-turn — independent of the player's action. Each is a Bernoulli trial in catalog order; the **first** that fires wins (≤1 special/counter). Returns the fired `MonsterSpecialMove` or `null`. Surfaced in the overlay/log/monster card.                                                                                                                                                                                                                      |
| `monsterSpecialDrainHeal(special, damageDealt)`                                | HP a monster heals from a `drain` special, `ceil(damageDealt × pct/100)`; `0` for any other special. Folded into the same HP cap as the `vampiric` passive in every resolver.                                                                                                                                                                                                                                                                                                                               |
| `effectiveAttackType(monster, special?)`                                       | The counter's effective school — `'magic'` when a `burst` special fired, else the monster's `attackType` (default `'physical'`). Drives both the mitigation math and the 🔮/⚔️ tag.                                                                                                                                                                                                                                                                                                                         |
| `isTelegraphedSpecial(effect)`                                                 | `true` for `heavy` / `burst` / `stun` — these wind up a round early (see `resolveCounterSpecial`); `pierce` / `drain` fire instantly. Lets the resolver decide prime-vs-fire.                                                                                                                                                                                                                                                                                                                               |
| `resolveCounterSpecial(monster, charging, rng?)` → `CounterSpecialResolution`  | Decides a counter's special given any held charge: a **charged** special fires now (`effective`); a freshly-rolled **telegraphed** one only winds up (`nextCharging` + `primed`); an **instant** one applies now. Pure (takes `monster` + `monsterCharging`, not `FightState`). Consumed by all three offensive resolvers.                                                                                                                                                                                  |
| `resolveStunnedSkipAction(input)`                                              | The player is stunned: forfeit the turn, the monster lands one **undefended** free hit (Rogue dodge applies, **no special roll** — no chain-stun), and `playerStunned` clears. Emits a `'stunned'` round; no overlay (instant commit). Driven by the action bar's stunned panel.                                                                                                                                                                                                                            |
| `checkMonsterFlee(monster, hp, maxHp, outcome, modifiers, stunApplied, rng?)`  | Whether a skittish low-HP monster bolts this round: fires only when the fight continues, the monster has a `flee` profile, it survived at/below `thresholdPct` of max HP, RNG lands under `chance`, fleeing isn't disabled (`monsterFleeDisabled` — dungeon rooms), and no stun landed. Sets `FightState.monsterFleeing`.                                                                                                                                                                                   |
| `rollFleeIntercept(character, monster)`                                        | The player's one chase roll at a fleeing monster: `d10 + AGI + escape bonuses` vs the monster's flee `d10`. Returns `{ caught }` — out-roll it for an instant kill, else it escapes. Mirrors `rollRunAway`.                                                                                                                                                                                                                                                                                                 |
| `resolveInterceptAction(input)`                                                | Resolves the intercept: **caught** → monster slain, `outcome: 'win'` + a full `rollLoot` (→ `onVictory`); **missed** → `outcome: 'fled'`, no rewards (→ `onFlee`). Emits an `'intercept'` round; animated via the two-die run display.                                                                                                                                                                                                                                                                      |
...
| `rollSpellCrit(spirit)`                                                        | Rolls a Spirit crit using `spellCritChance(spirit)`. Returns `{ crit, multiplier }`. Wired into `resolveAttackAction` / `resolveAbilityAction` / `resolveSpellAction` — fires only on a successful, damage-dealing action (basic attacks crit too as of the combat-consistency pass). Mirrored in `functions/src/gameLogic/combat.ts`.                                                                                                                                                                      |
...
| `resolveAttackAction(input, 'attack' \| 'magic')` | Standard d10 attack. Applies outgoing passives, **Spirit crit (`rollSpellCrit`)**, Soul Drain, `resolveRoundOutcome`. Runs all four `CombatModifiers` hooks in order. Pending payload carries the resolved monster counter (`monsterDamage`, `spiritCrit`) for the overlay.                                         |
| `resolveAbilityAction(input)`                     | 6d6 class ability roll. Applies subclass mods, Lethal Opener, lifesteal, Execute, Momentum stamina restore, fizzle refund. Pending payload now exposes the full monster counter (`monsterRoll`, `monsterStunned`, `monsterDamage`, `dodged`, `monsterAttackType`) so the ability overlay can show the enemy strike. |
...
| `resolveMeditateAction(input)`                    | Roll d10 + **effective WIS** magic restore (the class wisdom multiplier applies). Monster gets a free attack (defense bypassed).                                                                                                                                                                                    |
