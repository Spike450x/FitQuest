...
| `playerMaxHp(character)`                               | `50 + effStamina*2 + effHealth + gear` (effective = class-scaled). Mirrored in `functions/` (pool parity).                                                                                                                                                                                                                                                                                                                                                                 |
...
| `effectivePlayerDefenseVsMonster(char, monster)`       | Player effective DEF: class-scaled base DEF (`effectiveStat`) + flat gear − `monsterArmorPierce`. The DEF multiplier here is the **incoming damage affinity** (Wizard ×0.7 = fragile, Warrior ×1.5 = tanky). Used by `calculateRound` / `rollRunAway` / spell counter / `rollMonsterAttack`.                                                                                                                                                                               |     |
