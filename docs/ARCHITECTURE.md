...
| `components/`             | Shared UI building blocks (forms, cards, modals, bars). Key primitives: `Card` (variant system), `InputField` (canonical themed input — use instead of raw `<input>`), `Button`, `Modal`, `Skeleton`, `XPBar`, `GoldDisplay`, `SpellCard`, `EntityArt`.                                                                                                                                                                                                                   |
