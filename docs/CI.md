...
| 13  | Run E2E suite (composite action)   | `uses: ./.github/actions/run-e2e-suite`                                                   | Composite action that: builds Cloud Functions (with `functions/lib/` cache), installs Chromium for Playwright, boots `auth,firestore,functions` emulators on ports 9099/8080/5001, and runs `npx playwright test` with `NEXT_PUBLIC_USE_FIREBASE_EMULATOR=true`. Three Playwright projects run: `unauthenticated` (smoke + dark-mode), `authenticated` (route-renders smoke against the seeded test user), and `authenticated-flows` (real user actions — log a workout, claim a quest, win a fight, buy+equip gear, enter a dungeon). The composite is reused by `scheduled-e2e.yml`. `test-results/results.json` is emitted for the Discord notifier. |
