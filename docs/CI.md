...
| 4b  | Validate documentation counts      | `node scripts/validate-doc-counts.mjs`                                                     | Documentation drift. Computes headline counts from source (items 149 + by-type, spells 35 + by-class, monsters 21, achievements 31, item silhouettes) and fails the build if README.md / GAME-LOGIC.md / ART-ASSETS.md no longer match. Anchored on stable phrasing so historical CHANGELOG entries are never matched. When code changes a count, update the doc prose — not the script. See [GAME-LOGIC.md](GAME-LOGIC.md).                                                                                                                                                                                                                                                                                                       |
