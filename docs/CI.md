...
| 10  | Test                                        | `npm test`                                                                                                       | Vitest unit suite (49 files, 653 tests, ~12 s). Covers pure game logic (`src/lib/gameLogic/`), Zustand stores (`src/store/`), data-layer wrappers (`src/lib/`), hooks (`src/hooks/`), and UI components (`src/components/ui/`). JSX in `.tsx` test files is parsed via `@vitejs/plugin-react`. Coverage scope (`npm run test:coverage`) reports for all four areas; thresholds (80/80/70/80) gate only `src/lib/gameLogic/**`.                         |
