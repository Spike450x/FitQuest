...
| `src/components/art/item-silhouettes.tsx` | Hand-authored SVG silhouettes in `ITEM_SILHOUETTES` — 114 unique per-`id` silhouettes (one for every non-spell item) plus 4 type-level fallbacks (`weapon`/`armor`/`accessory`/`consumable`) as safety nets. Split out so non-item routes (combat / character / dashboard) don't pay the cost in their shared chunk. |
...
- **Per-item art** ✅ — All 114 non-spell items have unique per-`id` silhouettes (41 weapons, 26 armor, 31 accessories, 16 consumables). The 35 spells use the `SPELL_SILHOUETTES` map by spell effect key. Type-level fallbacks (`weapon`/`armor`/`accessory`/`consumable`) remain in `ITEM_SILHOUETTES` as safety nets for catalog additions before a dedicated silhouette is authored, and the dev-time `console.warn` in `EntityArt` flags any item id missing a dedicated silhouette. The `npm run validate:docs` guard fails CI if an item ever ships without one.
