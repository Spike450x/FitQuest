...
- **Reputation rank-up celebration + title flair** — new `RankUpCelebration` (`src/components/character/RankUpCelebration.tsx`) mirrors `LevelUpCelebration`: mounted once in the game layout, it watches the player's Reputation rank (derived from the monotonic `lifetimeReputation`) and pops a violet/fuchsia cinematic modal (Award glyph, confetti, `levelUp` sound) when the rank advances a tier, announcing the newly-unlocked title. A baseline-on-first-observation guard prevents a false trigger when a higher rank loads from Firestore on first paint. The modal offers a **one-tap "Equip" button** that sets `activeTitle` inline (with a "Not now" escape) so players don't have to detour to the character sheet. The equipped title (`resolveActiveTitle`) shows as an italic flourish under the name on the **dashboard hero** card and on the **arena + hunt victory banners** ("{name}, '{title}'") — a reputation you carry into the fight.
